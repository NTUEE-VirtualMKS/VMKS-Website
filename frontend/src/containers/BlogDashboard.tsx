...
const BlogDashboard: React.FC = () => {
  ...
  const displayArticles = filteredArticles.length > 0 ? filteredArticles : data?.GetAllArticles?.articles;
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {displayArticles?.filter(Boolean)?.map((article) => (
          <div key={article!.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105">
            <Link to={`/post/${article!.id}`}>
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500 dark:text-gray-400">{new Date(article.time).toLocaleDateString()}</span>
                </div>
              </div>
            </Link>
