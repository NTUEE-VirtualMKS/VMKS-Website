      ...
      <>
         <div className="flex flex-col grow px-9 pt-5 pb-5 w-full font-semibold border border-blue-600 border-solid bg-black bg-opacity-50 rounded-[30px] max-md:px-5 max-md:mt-6 max-md:max-w-full">
            <div className="flex gap-5 justify-between max-md:flex-wrap max-md:max-w-full">
                ...
                <button className="self-end mt-9 text-sm text-right text-white text-opacity-50" onClick={()=>navigate("AnnouncementPage")}>
                    See all
                </button>
            </div>
            ...
            {announcements
                ...
                .map((announcement) => (
                announcement &&(
                <div key={announcement.id} className="flex flex-col gap-5 w-full rounded-lg border p-3 text-xs text-white text-opacity-50 max-md:flex-wrap max-md:max-w-full">
                    <div className="flex flex-col justify-between">
                        <div className="self-start text-white text-base"><b>{announcement.title}</b></div>
